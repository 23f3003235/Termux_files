...
class LifeTrackerHandler(SimpleHTTPRequestHandler):
                ...
                if os.path.exists("notifications.json"):
                    with open("notifications.json", "r") as f:
                        ...
                        if content:
                            ...
                            specific_date = settings.get('specific_date_notification')
    ...
    def send_daily_report_notification(self):
        """Send daily time utilization report notification using subprocess"""
        try:
            if not os.path.exists("lifetracker.csv"):
                return
    ...
    def send_consistency_reminder(self):
        ...
        try:
            ...
            # Check if activity was logged today using subprocess for file reading
            today = datetime.now().strftime("%d-%m-%Y")
            try:
                result = subprocess.run(
                    ['grep', '-c', today, "lifetracker.csv"],
                    capture_output=True,
                    text=True,
                    timeout=5
                )
                has_today_activity = result.returncode == 0 and int(result.stdout.strip()) > 0
            except:
                # Fallback to manual reading if grep fails
                with open("lifetracker.csv", "r") as f:
                    reader = csv.reader(f)
    ...
    def test_notification(self):
        """Test notification endpoint with subprocess"""
        try:
            content_length = int(self.headers['Content-Length'])
            post_data = self.rfile.read(content_length)
            data = json.loads(post_data.decode())
    ...
    def send_custom_notification(self):
        """Send custom notification immediately using subprocess"""
        try:
            content_length = int(self.headers['Content-Length'])
            post_data = self.rfile.read(content_length)
            data = json.loads(post_data.decode())
    ...
    <script>
        ...
        async function loadNotificationSettings() {
            try {
                ...
                document.getElementById('notificationResult').innerHTML = '<div class="result error">Error loading settings: ' + error + '</div>';
            }
        }
        ...
        async function saveNotificationSettings() {
            ...
            try {
                ...
                document.getElementById('notificationResult').innerHTML = '<div class="result error">Error saving settings: ' + error + '</div>';
            }
        }
        
        function resetNotificationSettings() {
            if (confirm('Reset all notification settings to defaults?')) {
                document.getElementById('dailyReportEnabled').checked = false;
                document.getElementById('dailyReportTime').value = '09:00';
                document.getElementById('todoReminderEnabled').checked = false;
                document.getElementById('todoReminderTime').value = '18:00';
                document.getElementById('goalReminderEnabled').checked = false;
                document.getElementById('goalReminderTime').value = '20:00';
                document.getElementById('specificDateEnabled').checked = false;
                document.getElementById('specificDate').value = '';
                document.getElementById('specificDateTime').value = '10:00';
                document.getElementById('specificDateTitle').value = '';
                document.getElementById('specificDateMessage').value = '';
                document.getElementById('consistencyReminderEnabled').checked = false;
                document.getElementById('consistencyReminderTime').value = '21:00';
                
                document.getElementById('notificationResult').innerHTML = '<div class="result success">Settings reset to defaults</div>';
            }
        }
            ...
            try {
                const response = await fetch('/test_notification', {
                    ...
                    body: JSON.stringify({
                        ...
                        message: 'This is a test notification from Life Tracker! If you can see this, notifications are working correctly.'
                    })
                });
                ...
                document.getElementById('notificationResult').innerHTML = '<div class="result error">Test failed: ' + error + '</div>';
            }
        }
        ...
        async function sendCustomNotification() {
            ...
            try {
                ...
                document.getElementById('notificationResult').innerHTML = '<div class="result error">Error: ' + error + '</div>';
            }
        }
