...
class LifeTrackerHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == '/':
            ...
            return
        elif self.path.startswith('/open_clock'):
            self.open_clock_with_type()
            return
        return SimpleHTTPRequestHandler.do_GET(self)
    
    def open_clock_with_type(self):
        """Open Android Clock app with specific function - multiple approaches"""
        try:
            # Get the type from query parameter
            if '?' in self.path:
                params = self.path.split('?')[1]
                clock_type = params.split('=')[1] if '=' in params else 'main'
            else:
                clock_type = 'main'
            
            # Try multiple package names and approaches
            packages = [
            ...
            for package in packages:
                try:
                    if clock_type == 'main':
                        # Try different main activity patterns
                        activities = [
                            f"{package}/.DeskClock",
                            f"{package}/com.android.deskclock.DeskClock",
                            f"{package}/.AlarmClock",
                            f"{package}/.clock"
                        ]
                        ...
                        for activity in activities:
                            ...
                            if result.returncode == 0:
                                success = True
                                output = f"Successfully opened with: {activity}"
                                break
                            else:
                                output += f"Failed with {activity}: {result.stderr}\n"
                        
                        if success:
                            break
                            
                    elif clock_type == 'alarm':
                        # Try alarm-specific intents
                        result = subprocess.run(
                        ...
                        if result.returncode == 0:
                            success = True
                            output = "Successfully opened alarm"
                            break
                            
                    elif clock_type == 'timer':
                        # Try timer-specific intents
                        result = subprocess.run(
                        ...
                        if result.returncode == 0:
                            success = True
                            output = "Successfully opened timer"
                            break
                            
                    elif clock_type == 'stopwatch':
                        # Try opening main clock and hope it goes to stopwatch
                        result = subprocess.run(
                            ["am", "start", "-n", f"{package}/.DeskClock", "--es", "fragment", "stopwatch"],
                            capture_output=True,
                            text=True,
                            timeout=3
                        )
                        if result.returncode == 0:
                            success = True
                            output = "Successfully opened stopwatch"
                            break
                            
                except Exception as e:
                    output += f"Error with {package}: {str(e)}\n"
                    continue
            
            # If nothing worked, try generic clock intent
            if not success:
                result = subprocess.run(
                ...
                if result.returncode == 0:
                    success = True
                    output = "Opened dialer (fallback)"
                else:
                    output += "All clock app attempts failed"
...
<body>
    <div class="container">
        ...
        <div class="button-group">
            <button class="preview-btn" onclick="openClockApp('main')">
                Open Clock App
            </button>
            <button class="preview-btn" onclick="openClockApp('alarm')">
                Set Alarm
            </button>
            <button class="preview-btn" onclick="openClockApp('timer')">
                Open Timer
            </button>
            <button class="preview-btn" onclick="openClockApp('stopwatch')">
                Open Stopwatch
            </button>
        </div>
        <div id="clockResult" style="margin-top: 15px;"></div>
        <script>
            function openClockApp(type) {
                const resultDiv = document.getElementById('clockResult');
                
                if (resultDiv) {
                    resultDiv.innerHTML = '<div class="result info"><span class="loading"></span>Opening ' + type + '...</div>';
                }
                
                fetch('/open_clock?type=' + type)
                    .then(response => response.json())
                    .then(data => {
                        if (resultDiv) {
                            if (data.status === 'success') {
                                resultDiv.innerHTML = '<div class="result success">✓ ' + type + ' opened successfully!</div>';
                            } else {
                                resultDiv.innerHTML = '<div class="result error">Failed: ' + (data.output || 'Unknown error') + '</div>';
                            }
                        }
                    })
                    .catch(error => {
                        if (resultDiv) {
                            resultDiv.innerHTML = '<div class="result error">Network error: ' + error + '</div>';
                        }
                    });
            }
        </script>
