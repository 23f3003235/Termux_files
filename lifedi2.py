...
class LifeTrackerHandler(SimpleHTTPRequestHandler):
    ...
    def handle_ai_chat(self):
        ...
        deepinfra_api_key = "WBgAzuUrqu20i6thQrFHs4xKAdPeJpwc"
        
        # Test the API key first
        test_headers = {"Authorization": f"Bearer {deepinfra_api_key}"}
        ...
        try:
            ...
            if test_response.status_code == 200:
                ...
                print("Available models:", [model['name'] for model in models[:5]])
                
                # Now try chat with a known working model
                endpoint = "https://api.deepinfra.com/v1/openai/chat/completions"
                data = {
                    "model": "meta-llama/Llama-2-7b-chat-hf",  # Usually works
                    "messages": [{"role": "user", "content": user_message}],
                    "max_tokens": 100
                }
                
                chat_response = requests.post(endpoint, headers=test_headers, json=data, timeout=30)
                ...
                if chat_response.status_code == 200:
                    ...
                    reply = f"Chat failed: {chat_response.status_code}"
            else:
                reply = f"API key test failed: {test_response.status_code}"
                
        except Exception as e:
            reply = f"Test failed: {str(e)}"
