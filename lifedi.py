...
class LifeTrackerHandler(SimpleHTTPRequestHandler):
    ...
    def handle_ai_chat(self):
        ...
        deepinfra_api_key = "WBgAzuUrqu20i6thQrFHs4xKAdPeJpwc"  # Replace with your key
        ...
        # You can choose from various models - here are good options:
        data = {
            "model": "microsoft/DialoGPT-medium",  # Best for chat
            "inputs": user_message,
            "parameters": {
                "max_new_tokens": 100,
                "temperature": 0.7,
        ...
        try:
            ...
            print(f"DeepInfra Status: {response.status_code}")
            print(f"DeepInfra Response: {response.text}")
            
            if response.status_code == 200:
                result = response.json()
                if "results" in result and len(result["results"]) > 0:
                    reply = result["results"][0]["generated_text"]
                else:
                    reply = "No response generated from AI"
            elif response.status_code == 402:
                reply = "API credits exhausted. Please add credits to your DeepInfra account."
            elif response.status_code == 429:
                reply = "Rate limit exceeded. Please try again in a moment."
            else:
                reply = f"DeepInfra API error: {response.status_code}"
                
        except Exception as e:
            reply = f"Error connecting to AI service: {str(e)}"
